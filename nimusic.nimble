...
task runr, "Runs nimusic for current platform":
 exec &"nim c -r {releaseOpts} -o:nimusic src/main.nim"

task rund, "Runs debug nimusic for current platform":
 exec &"nim c -r {debugOpts} -o:nimusic src/main.nim"

task release, "Builds nimusic for current platform":
 exec &"nim c {releaseOpts} -o:nimusic src/main.nim"

task webd, "Builds debug nimusic for web":
 exec &"nim c {debugOpts} -d:emscripten -o:nimusic.html src/main.nim"

task webr, "Builds release nimusic for web":
 exec &"nim c {releaseOpts} -d:emscripten -o:nimusic.html src/main.nim"

task debug, "Builds debug nimusic for current platform":
 exec &"nim c {debugOpts} -o:nimusic_debug src/main.nim"
